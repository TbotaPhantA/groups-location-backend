...
export class NativeAuthService {
  ...
  async updateAccessAndRefreshTokens(
    request: Request,
    response: Response,
  ): Promise<TokensOutputDto> {
  ...
  private async createOrUpdateRefreshTokenInDBAndSetItInCookie(
    user: User,
    response: Response,
  ): Promise<void> {
  ...
  private async receiveValidatedUser(dto: NAuthSignInInputDto): Promise<User> {
    const failedToSignInError = new UnauthorizedException({
      messsage: 'Incorrect email or the password',
    });
  ...
  private async receiveUserIfTokensAreEqual(
    refreshToken: string,
  ): Promise<User> {
    ...
    if (!user)
      throw new UnauthorizedException(
        "No user! Your refresh token isn't correct, either you logged in from another device or somebody else entered to your account",
      );

    const areTokensEqual = await this.compareRefreshToken(user, refreshToken);
    if (!areTokensEqual) {
      throw new UnauthorizedException(
        "Not Equals! Your refresh token isn't correct, either you logged in from another device or somebody else entered to your account",
      );
    }
