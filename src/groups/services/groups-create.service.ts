...
export class GroupsCreateService {
  ...
  async createGroup(user: User, dto: CreateGroupInputDto) {
    ...
    const groupWithoutMembers = await this.groupRepository.save(
      groupWithoutMembersDto,
    );

    const userGroup = await this.createUserGroup(user, groupWithoutMembers);

    const group = await this.groupRepository.findOne(groupWithoutMembers.uuid, {
      relations: ['owner', 'usersGroups'],
    });
    return group;
  }
  ...
  async createInviteLink(
    ...
    const redisNewGroupInviteValue = await this.redisService.createGroupInvite(
      uuidOfGroup,
    );
