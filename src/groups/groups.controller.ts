...
export class GroupsController {
  constructor(
    private groupsCreateService: GroupsCreateService,
  ...
  @ApiResponse({
    status: HttpStatus.CREATED,
    description:
      'if group with this uuid is exist, responds with information about this group',
    type: Group,
  })
  @Get(':uuid')
  ...
  @ApiResponse({
    ...
    description:
      'If group name was updated successfully, 204 status code with be returned, with no any content',
  })
  @UsePipes(ValidationPipe)
  @ApiBearerAuth()
  @GroupRole('owner')
  ...
  async updateGroupName(
    ...
    @Body() dto: UpdateGroupNameInputDto,
  ): Promise<void> {
  ...
  @ApiResponse({
    status: HttpStatus.OK,
    description:
      'If currenter user is the member of the group creates disposalbe invitation link',
  ...
  createInviteLink(
    @Param('uuid') uuid: string,
  ): Promise<CreateInviteLinkOutputDto> {
    return this.groupsCreateService.createInviteLink(uuid);
  }

  @Put('useInvite/:groupUUID/:inviteKey')
  @ApiOperation({
    summary: 'If inviteKey is valid, adds current user to group members',
  })
  @ApiResponse({
    status: HttpStatus.OK,
